...
def gendef gen_agl_map(inputs, height, width, feature_dims):
    with tf.name_scope("gen_agl_map"):
        batch_size = tf.shape(inputs)[0]
        ret = tf.reshape(tf.tile(inputs, [1, height * width]), [batch_size, height, width, feature_dims])
        return ret
...
def inference(input_img, input_fp, input_agl, phase_train, conf):
    ...
    with tf.name_scope('warping_model'):
        ...
        with tf.name_scope('warping_module'):
            '''coarse module'''
...
def dist_loss(y_pred, y_, method="MAE"):
    with tf.name_scope('img_dist_loss'):
        loss = 0
        if method == "L2":
...
def center_weight(shape, base=0.005, boundary_penalty=3.0):
    with tf.name_scope('center_weight'):
